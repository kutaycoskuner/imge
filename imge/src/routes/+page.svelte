<script lang="ts">
    ...
    function calculateSum(matrixA: number[][], matrixB: number[][]): number[][] {
        return matrixA.map((row, r) =>
            row.map((val, c) => val + (matrixB?.[r]?.[c] ?? 0))
        );
    }
...
.row-2 { font-size: 2rem; }
.row-15 { font-size: 1.5rem; }
.row-1 { font-size: 1rem; }
...
.color-system-a { color: #0076ff; }
.color-system-a .cell:hover { background: rgba(0, 118, 255, 0.1); border-color: rgba(0, 118, 255, 0.3); }
.color-system-a .cell-input { border: 2px solid #0076ff; }
:global(.app-container) .group-a input[type="number"]:focus { border-color: #0076ff; outline: none; }

/* Channel B: Matrix B (Deep Royalty Purple) */
.color-system-b { color: #7928ca; }
.color-system-b .cell:hover { background: rgba(121, 40, 202, 0.1); border-color: rgba(121, 40, 202, 0.3); }
.color-system-b .cell-input { border: 2px solid #7928ca; }
:global(.app-container) .group-b input[type="number"]:focus { border-color: #7928ca; outline: none; }

/* Channel C: Result Matrix C (Vibrant Emerald Green) */
.color-system-c { color: #00a36c; }
</style>